...
const Widgets = ({ newsResults, randomUsersResults }) => {
  const [articleNumber, setArticleNumber] = React.useState(3);
  const [randomUserNumber, setRandomUserNumber] = React.useState(3);
  return (
    <div className="xl:w-[350px] hidden lg:inline ml-8 space-y-5 ">
      ...
      <div
        className="text-gray-700 space-y-3 bg-gray-100 rounded-xl
      pt-2 w-[90%] xl:w-[75%]]"
      >
      ...
      <div className="sticky top-16  text-gray-700 space-y-3 bg-gray-100 pt-2 rounded-xl w-[90%] xl:w-[90%]">
        ...
        {randomUsersResults.slice(0, randomUserNumber).map((randomUser) => (
          ...
          >
            ...
            <button
              className="ml-auto bg-black text-white rounded-full text-sm 
        ...
        <button
          className="text-blue-300 pl-4 pb-3 hover:text-blue-400"
          onClick={() => setRandomUserNumber((prev) => prev + 3)}
        >
          Show more
        </button>
      </div>
